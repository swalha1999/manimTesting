...
"""
    ...
    def generate_code_string(self):
        ...
        curr = self.head_state.next
        while curr.next is not None:  # is not the END state
            code += self.generate_state_code(curr)
            curr = curr.next

        if self.write_apply_function:
            code += self.APPLYFUNCTION
    ...
    def generate_state_code(self, curr):
        code = ""
        code += self.generate_target_equivalences(curr)
        code += self.generate_removed(curr)
        code += self.generate_mobject_functions(curr)
        code += self.generate_targets(curr)
        code += self.generate_modified_added(curr)
        code += self.generate_animations(curr)
        return code

    def generate_target_equivalences(self, curr):
        code = ""
        for anim in curr.animations:
            if anim.imobject.is_deleted:
                continue
    ...
    def generate_removed(self, curr):
        code = ""
        for imobject in curr.removed:
            if imobject.is_deleted:
                continue
            code += f"        self.remove({mh.get_name(imobject)})\n"
        if curr.removed:
            code += "\n"
        return code

    def generate_mobject_functions(self, curr):
        code = ""
        for imobject in curr.called_mobject_functions:
            if imobject.is_deleted:
                continue
            for func, args in curr.called_mobject_functions[imobject].items():
                ...
                mobj_name = mh.get_name(imobject)
                code += f"        {mobj_name}.{func}({', '.join(args_names)})\n"
        return code

    def generate_targets(self, curr):
        code = ""
        target_strs = []
        
        # Generate VGroup targets first
        for imobject in curr.targets:
            if (
                imobject.is_deleted
                or not isinstance(imobject.mobject, VGroup)
                ...
                continue
            target_strs.append(self.generate_target_init(curr, imobject))
        ...
        for imobject, tobj_str in target_strs:
            code += self.generate_target_manip(curr, imobject, tobj_str)

        for imobject in curr.targets:
            if imobject.is_deleted or not isinstance(imobject, IDependent):
                continue

            imobject, tobj_str = self.generate_target_init(curr, imobject)
            code += self.generate_target_manip(curr, imobject, tobj_str)
    ...
    def generate_target_init(self, curr, imobject):
        ...
        target_decl = curr.target_decl_str[imobject]
        code = f"        {tobj_str} = {target_decl}\n"

        return imobject, tobj_str

    def generate_target_manip(self, curr, imobject, tobj_str):
        code = ""
        for func, args in curr.called_target_functions[imobject].items():
            args_names = [
                (
            ...
            code += f"        {tobj_str}.{func}({', '.join(args_names)})\n"
        return code

    def generate_modified_added(self, curr):
        code = ""
        for imobject in curr.targets:
            if imobject.is_deleted:
                continue
            if imobject in curr.added and imobject.intro_anim is None:
                code += f"        self.add({mh.get_name(imobject)})\n"
        return code

    def generate_animations(self, curr):
        anim_strs = [
            self.get_anim_str(anim, curr)
            for anim in curr.animations
            if not anim.imobject.is_deleted
        ...
        run_time = f", run_time={str(curr.run_time)}" if curr.run_time != 1.0 else ""
        if anim_strs:
            return f"        self.play({', '.join(anim_strs)}){run_time}\n"
        else:
            return f"        self.wait({curr.run_time})\n"
